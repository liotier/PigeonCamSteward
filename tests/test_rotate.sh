...
# rotation. Drives the real nestcam-rotate.sh with a fake systemctl
...
out=$(
    ...
    NESTCAM_ROTATE_SETTLE_DELAY=1 \
    "$REPO_ROOT/bin/nestcam-rotate.sh" 2>&1
)
rc=$?
assert_eq "0" "$rc" "rotate.sh exits 0 on a normal restart-mode rotation"

mapfile -t calls < <(grep -E 'stop nestcam-stream|start nestcam-stream' "$SYSTEMCTL_LOG")
assert_eq "2" "${#calls[@]}" "exactly one stop and one start were issued"
assert_contains "${calls[0]:-}" "stop nestcam-stream.service" "the stop call happens first"
assert_contains "${calls[1]:-}" "start nestcam-stream.service" "the start call happens second"
...
mapfile -t calls2 < <(grep -E 'stop nestcam-stream|start nestcam-stream' "$SYSTEMCTL_LOG")
...
out3=$(PATH="$FAKE_BIN:$PATH" NESTCAM_CONFIG="$CONFIG_API" FAKE_SYSTEMCTL_LOG="$SYSTEMCTL_LOG" \
    "$REPO_ROOT/bin/nestcam-rotate.sh" 2>&1)
rc3=$?
...
assert_eq "0" "$(grep -cE 'stop nestcam-stream|start nestcam-stream' "$SYSTEMCTL_LOG" 2>/dev/null || true)" \
